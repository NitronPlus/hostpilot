...
pub fn handle_link(config: &Config, alias: String) -> Result<()> {
    ...
    // Prefer ssh-copy-id if available (better handling on remote)
    ...
    // Fallback: send key via ssh stdin and run remote safe append script
    let remote_script = r#"
...
"#;
    ...
    if let Some(mut stdin) = child.stdin.take() {
        ...
        // send key followed by the remote script
...
// scp-based transfer command has been removed; use external scp or the built-in `ts` SFTP in `transfer.rs`.

// SFTP-based transfer (stsf -> ts) implementation moved to `src/transfer.rs` as `handle_ts`.
...
pub fn handle_set(
    ...
    // write back to config file (use default location)
    cfg.save_to_storage();
    println!("✅ 配置已更新");
    Ok(())
}
